...
class _AntrianScreenState extends State<AntrianScreen> {
  ...
  Future<void> _showFinishConfirmationDialog(String invoice) async {
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Konfirmasi'),
        content:
            const Text('Apakah kamu yakin ingin menyelesaikan pesanan ini?'),
        actions: [
          TextButton(
  ...
  Future<void> _finisOrder(String invoice) async {
    try {
      await OrderService.updateStatusInvoice(invoice, {
        'is_finished': true,
      });
              ...
              children: [
                ...
                if (isPaid)
                  OutlinedButton.icon(
                    ...
                    onPressed: () {
                      _showFinishConfirmationDialog(order.invoice);
                    },
                  ),
              ],
            ),
